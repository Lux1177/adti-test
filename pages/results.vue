	...
	<div>
		...
		<div v-if="quizStore.state.isLoading" class="flex justify-center items-center min-h-screen">
			<div class="text-center space-y-6 animate-fade-in-scale">
				...
				<p class="text-2xl text-white font-light">{{ t('loading.results') }}</p>
			</div>
		</div>
		...
		<div v-else-if="!hasResults" class="flex justify-center items-center min-h-[calc(100vh-5rem)]">
			<div class="text-center space-y-8 animate-fade-in-scale">
				<div class="glass-ultra rounded-3xl p-12 shadow-2xl card-ultra-hover max-w-lg">
					...
					<p class="text-white/70 mb-8 text-lg leading-relaxed">{{ t('alert.complete_test_first') }}</p>
					<NuxtLink to="/">
		...
		<div v-else class="max-w-6xl mx-auto animate-fade-in-scale">
			<div class="glass-ultra p-6 md:p-8 shadow-2xl rounded-3xl card-ultra-hover">
				...
				<div class="text-center mb-8 animate-fade-in-up">
					...
					<div class="glass-light-ultra rounded-3xl p-6 border border-white/10 mb-8 card-ultra-hover">
						<div class="text-2xl md:text-3xl text-white/90 mb-4 font-light">
							{{ t('results.score', { score: score, total: totalQuestions }) }}
						</div>
						<div class="text-5xl md:text-6xl font-light mb-6" :class="getScoreColorClass()">
				...
				<div class="mb-8 animate-fade-in-up animate-stagger-2">
					<h3 class="text-2xl font-light text-white mb-8 text-center">{{ t('results.details') }}</h3>
					...
					<div class="max-h-96 overflow-y-auto space-y-6 custom-scrollbar">
						...
						>
							<div
								...
								:key="index"
								class="glass-light-ultra rounded-3xl p-6 border transition-all duration-500 card-ultra-hover"
				...
				<div class="flex flex-col sm:flex-row gap-6 justify-center animate-fade-in-up animate-stagger-3">
					<button
						@click="restartQuiz"
						class="btn-ultra-smooth px-8 py-4 text-lg text-white font-semibold rounded-3xl flex items-center justify-center micro-bounce"
					>
					...
					<NuxtLink to="/">
						<button class="btn-ultra-smooth px-8 py-4 text-lg text-blue-300 font-semibold rounded-3xl flex items-center justify-center w-full sm:w-auto micro-bounce">
...
const getScoreText = () => {
	if (percentage.value >= 90) return t('score.excellent')
	if (percentage.value >= 80) return t('score.very_good')
	if (percentage.value >= 70) return t('score.good')
	if (percentage.value >= 60) return t('score.average')
	return t('score.poor')
}
...
onMounted(() => {
	// Don't try to load questions on results page - we should already have the data
	// Just load the saved state
	const hasState = quizStore.loadState()

	// If no state is found, redirect to home
	if (!hasState) {
		router.push('/')
	}
})
